...
PERSONA_STYLES: Dict[str, Dict[str, str]] = {
    "friendly": {
        ...
        "instruction_en": "Speak naturally, warmly, and clearly. Sound like an attentive person in a real conversation while remaining honest that you are an AI. Use varied sentences, answer the actual intent, and avoid canned introductions, fake emotion, excessive filler, and repeated conclusions.",
        "instruction_el": "Μίλα φυσικά, φιλικά και καθαρά, σαν προσεκτικός συνομιλητής σε πραγματική συζήτηση, παραμένοντας ειλικρινές ότι είσαι AI. Χρησιμοποίησε ποικίλες προτάσεις, απάντησε στην πραγματική πρόθεση και απόφυγε έτοιμες εισαγωγές, ψεύτικο συναίσθημα, περιττές φράσεις και επαναλαμβανόμενα συμπεράσματα.",
    },
    "calm_expert": {
    ...
    "mentor": {
        ...
        "instruction_en": "Teach patiently and conversationally. Build difficult ideas from prerequisites to mechanism to examples, check likely misunderstandings, and encourage understanding without sounding patronizing or mechanical.",
        "instruction_el": "Δίδαξε με υπομονή και φυσικό διάλογο. Χτίσε τις δύσκολες ιδέες από τα προαπαιτούμενα στον μηχανισμό και στα παραδείγματα, έλεγξε πιθανές παρανοήσεις και βοήθησε την κατανόηση χωρίς υποτιμητικό ή μηχανικό ύφος.",
    },
    "direct": {
...
def naturalize_response(text: str, language: str, config: Dict[str, Any], route: str = "", seed_text: str = "") -> str:
    ...
    if not cleaned:
        return cleaned
    exact_routes = {"calculator", "unit_conversion", "time", "date", "system", "language_reject", "natural_conversation", "universal_knowledge", "universal_followup", "encyclopedia_extract"}
    ...
    # Remove common model boilerplate while preserving substantive content.
    boilerplate = (
        r"(?i)^\s*(sure|certainly|of course|okay|here is the answer|here\'s the answer|let me explain)\s*[:—,-]*\s*",
        r"(?i)^\s*(βεβαίως|φυσικά|εντάξει|εδώ είναι η απάντηση|ας το εξηγήσουμε)\s*[:—,-]*\s*",
    )
    for pattern in boilerplate:
        cleaned = re.sub(pattern, "", cleaned).strip()
    cleaned = re.sub(r"(?im)^\s*(in conclusion|to summarize|overall),?\s*", "", cleaned)
    cleaned = re.sub(r"(?im)^\s*(συμπερασματικά|συνοψίζοντας),?\s*", "", cleaned)
    # Collapse duplicate adjacent headings/lines often produced by tiny models.
    rows=[]
    for line in cleaned.splitlines():
        if rows and line.strip() and line.strip().casefold()==rows[-1].strip().casefold():
            continue
        rows.append(line)
    cleaned="\n".join(rows).strip()
