...
def choose_consensus(question: str, first: str, second: str, first_quality: Dict[str, Any], second_quality: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    # The second pass is intentionally the stronger tier; apply only a small tie-breaker.
    second_score += 0.025
    selected = second if second_score >= first_score else first
    return selected, {
        "agreement": round(agreement, 4),
        "draft_score": round(first_score, 4),
        "verification_score": round(second_score, 4),
        "selected": "verification" if selected is second else "draft",
    }
