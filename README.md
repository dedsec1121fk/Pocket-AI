...
<details>
<summary><strong>Compact Hybrid Intelligence: 0.6B to 1.7B</strong></summary>

Pocket AI 15.0 begins normal local inference at **Qwen3 0.6B**, not 135M. It adds real intermediate tiers and selects the strongest safe installed model from live phone conditions.
...
Adjacent hybrids run sequentially: `0.6B→0.8B`, `0.8B→1.5B`, and `1.5B→1.7B`. The first model is unloaded before the stronger model starts. The parameters are **not added**; for example, 0.6B→0.8B is not a 1.4B model.
...
Το Pocket AI 15.0 ξεκινά κανονικά από πραγματικό μοντέλο **0.6B** και προσθέτει πραγματικά tiers 0.8B, 1.5B και 1.7B. Τα δύο 135M παραμένουν μόνο ως emergency fallback. Τα υβριδικά routes είναι διαδοχικά και οι παράμετροι δεν αθροίζονται.
...
* selected emergency 135M, 0.6B, 0.8B, 1.5B, 1.7B, 4B, or 8B model tier
...
The 135M models remain in verified split parts for offline and critical low-memory fallback. They are not the normal Pocket AI 15 starting tier.

### Sequential hybrids

Pocket AI can run adjacent installed models one after another: 135M→0.6B, 0.6B→0.8B, 0.8B→1.5B, 1.5B→1.7B, 1.7B→4B, or 4B→8B. It unloads the analyst before starting the stronger synthesizer, rechecks RAM and temperature, and never adds parameter counts.
...
* επιλεγμένο emergency 135M ή κανονικό tier 0.6B, 0.8B, 1.5B, 1.7B, 4B ή 8B
...
* Ισχυρότερες συσκευές 4–8 GB προχωρούν σε πραγματικά tiers 0.8B, 1.5B και 1.7B.
...
Η κανονική εγκατάσταση επιλέγει το ισχυρότερο ασφαλές tier. Με `bash "Other Files/install_models.sh" --compact` εγκαθίστανται όλα τα tiers από 0.6B έως 1.7B.
...
Τα routes 0.6B→0.8B, 0.8B→1.5B και 1.5B→1.7B εκτελούνται διαδοχικά. Το πρώτο μοντέλο κλείνει πριν ανοίξει το ισχυρότερο. Οι παράμετροι δεν αθροίζονται.
